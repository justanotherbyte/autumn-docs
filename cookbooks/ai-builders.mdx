...
Using Autumn, you can integrate Stripe with your AI application builder like [Lovable](https://www.lovable.dev), or [Bolt.new](https://https://bold.new) in just a couple prompts--no webhooks or server management required.

This guide is only for applications that use Supabase functions as their backend.
...
Now you can describe to the AI the pricing plans you defined in Autumn (eg free tier has 3 messages, pro tier is 5 usd per month etc). Make sure to provide it with the productIds and featureIds (if you're using usage pricing or limits).
...
3. Check that shadcn is installed. If it is not, ask the user if they are sure they want to proceed.
