...
Using Autumn, you can integrate Stripe with your AI application builder like [Lovable](https://www.lovable.dev), [Bolt.new](https://https://bolt.new) or [Replit](https://replit.com) in just a couple prompts--no webhooks or server management required.
...
1. Install autumn-js: `npm install autumn-js`
2. Mount the Autumn handler server-side, which will be used by Autumn’s frontend react hooks. The handler takes in an identify function that returns a customerId, which should be resolved from auth. The below is an example from better auth, but you should use Replit Auth
