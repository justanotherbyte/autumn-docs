...
<Accordion title="Create a Pricing page and call Autumn on purchase">
{/* <Note>
...
<Note>
  ...
  user ID from your database (which is likely Supabase).
</Note>
```plaintext Prompt
...
My free product has an id of ‘free’, and pro product has a product_id of ‘pro’ Use our user ID as customer_id from our DB. Please keep force_checkout as true so a URL is always returned.

Use this API key for Autumn. Leave it in code for now: <>
...
<Accordion title="Check if a user has access to pro-features">
<Note>
...
The feature_id is ‘pro-features’. Please use our user ID as customer_id from our DB.
