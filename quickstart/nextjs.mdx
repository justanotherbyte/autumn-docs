    ...
    <html>
      <body>
        <AutumnProvider
          ...
          customerData={{ name: "John Doe", email: "john@doe.com" }}
        >
          {children}
        </AutumnProvider>
      </body>
...
"use client"; // As Autumn is a client-side library
...
<Note>
  `autumn-js/next` is a client-side library, so the component that uses it must
