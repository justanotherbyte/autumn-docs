...
The pricing table component can take in a `productDetails` prop, which can be used to pass in content that can override the default contents (which come from your Autumn products). The example below was used to render the screenshot above.

By passing this prop, you can choose which products should be displayed, and what items should be displayed. The items array can either be an object with `primaryText` and `secondaryText` properties, or a string with the `featureId`, which will just take the item from Autumn's default contents.
