...
This creates routes in the `/api/autumn/*` path, which will be used by Autumn's frontend React [hooks](/api-reference/hooks/useAutumn). These routes interact with Autumn's API (eg `attach`, `check`, `track`, `customer`) to handle the billing logic.
...
<CodeGroup>
  ```bash .env (Next.js) NEXT_PUBLIC_AUTUMN_BACKEND_URL=http://localhost:3000
  ``` ```bash .env (Vite) AUTUMN_BACKEND_URL=http://localhost:8000 ```
</CodeGroup>
